...
public interface OrderDAO extends DAO<Order> {
    @Override
    @SqlUpdate("INSERT INTO orders VALUES ( :userId, :status, :deliveryMethod, :deliveryPrice, :createdAt, :updatedAt)")
    @GetGeneratedKeys("id")
    long insert(@BindBean Order order);

    @Override
    @SqlUpdate("UPDATE orders SET userId = :userId, status = :status, deliveryMethod = :deliveryMethod, " +
            "deliveryPrice = :deliveryPrice, createdAt = GETDATE(), updatedAt = :updatedAt WHERE id = :id")
    ...
    @Override
    @SqlQuery("SELECT * FROM orders ORDER BY <orderBy> <orderDir> OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY")
    ...
    @SqlQuery("SELECT * FROM orders WHERE userId = :userId ORDER BY createdAt desc OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY")
//    @SqlQuery("select * from orders")
    ...
    @SqlUpdate("UPDATE orders SET status = 3, updatedAt =  GETDATE() WHERE id = :id")
    void cancelOrder(@Bind("id") long id);

    @SqlQuery("SELECT COUNT(id) FROM orders")
    int count();

    @SqlUpdate("UPDATE orders SET status = 2, updatedAt =  GETDATE() WHERE id = :id")
    void confirm(@Bind("id") long id);

    @SqlUpdate("UPDATE orders SET status = 3, updatedAt =  GETDATE() WHERE id = :id")
    void cancel(@Bind("id") long id);

    @SqlUpdate("UPDATE orders SET status = 1, updatedAt =  GETDATE() WHERE id = :id")
