...
public interface CategoryDAO extends DAO<Category> {
    ...
    @Override
    @SqlQuery("SELECT * FROM category ORDER BY name DESC OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY")
    List<Category> getPart(@Bind("limit") int limit, @Bind("offset") int offset);



    @Override
    @SqlQuery("SELECT * FROM category ORDER BY <orderBy> <orderDir> OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY")
